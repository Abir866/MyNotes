...
function HomeScreen({ navigation }) {
  ...
  }, [addNoteData, updateNoteData ]);

  const renderItem = ({ item }) => (
    <TouchableOpacity onPress={() => updateNote(item) } style={tw`w-[98%] mb-0.5 mx-auto bg-purple-300 rounded-sm px-1`}> 
    ...
    <Provider store={store}>
      <NavigationContainer>
        <Stack.Navigator initialRouteName="Home">
          <Stack.Screen
            options={{
              headerStyle: tw`bg-purple-300 border-20`,
              headerTintColor: '#fff',
